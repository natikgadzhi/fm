...
2. **Plan** — Break the project into phases and tasks:
   ...
   - **Phase 1: Core** — Core data models, types, interfaces, JMAP client
...
3. **Create tasks** using `TaskCreate` for each work item, with:
   - Clear subject (imperative: "Implement email search command")
   - Detailed description with acceptance criteria, file paths, and dependencies
   - `activeForm` in present continuous ("Implementing email search command")
...
3. **Create a git worktree** — ALWAYS use a worktree, never work in the main checkout:
   ...
   git worktree add ../fm-task-N -b task-N-description
   cd ../fm-task-N
   ```
...
1. **Check out the PR branch** in a worktree:
   ```bash
   git worktree add ../fm-review-N origin/task-N-description
   cd ../fm-review-N
   ```
...
6. **Security review**:
   - Ensure API tokens are not logged or exposed
   - Check that user input is validated at CLI boundaries
...
8. **Clean up the worktree** after review:
   ```bash
   cd /path/to/main/checkout
   git worktree remove ../fm-review-N
   ```
